 ...
 */
describe('线上真实采样的回归', () => {
  it('V.PS · 联通线路：倒数第二个 1106ms 在下一桶到来前即可隐藏', () => {
    // 2026-09-19 的 30 分钟窗口；截图时右侧只有 103ms 这一个正常点，旧规则显示「隐藏尖峰 · 0」。
    const samples: SpikeSample[] = [
      { timestamp: 1789830536279, value: 97 },
      { timestamp: 1789830596536, value: 95 },
      { timestamp: 1789830668714, value: 99 },
      { timestamp: 1789830788884, value: 98 },
      { timestamp: 1789830909080, value: 1106 },
      { timestamp: 1789831029317, value: 103 },
    ]
    expect(detectSpikes(samples).flatMap((hidden, index) => (hidden ? [index] : []))).toEqual([4])
  })
