 ...
 * 采样上的高点可以隐藏，但要求内侧有足够的安静邻居——否则一个边缘点就把纵轴顶住了。
 *
...
describe('应当隐藏的形态', () => {
  ...
  it('窗口第一个或最后一个采样上的高点，内侧证据足够时也隐藏', () => {
    ...
    // 边缘只处理最外侧那一个采样：紧挨着它的那个点，一侧只剩一个邻居，证据不足，保留。
    expect(hiddenIndexes([1200, 1180, 201, 199, 202, 200, 203])).toEqual([0])
...
describe('应当保留的形态', () => {
  ...
  it('只剩一两个邻居的一侧不放行：点太少会把基线带偏', () => {
    // 高点在第二个采样：左侧只剩一个点，不足以定基线，保留。
