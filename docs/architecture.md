...
- 只渲染领域模型和显式状态。
- 冷启动时，首页与设置页的后端设置驱动区域等主后端 `/api/config` 成功或明确失败才解除骨架屏；详情页同时等待节点所属后端和主后端配置确定。已有真实配置的手动刷新继续显示旧快照。失败时维持既有错误提示并使用默认设置，不会无限等待；详情标题、版本和授权状态仍归节点所属后端，主题设置仍归主后端，不增加配置请求。
...
- `src/theme/site-theme-hint.ts` 另存非权威的上次站点明暗模式，只用于配置未确定时给首屏选择明暗；它不参与三层合并，也不进入草稿、统计、复制 JSON 或保存快照。store 初始化即读取并应用；确认配置后只以成功回读的后端模式更新，冷启动失败时本次回到默认（旧暂存保留供下次访问），本地覆盖始终优先。
- `DynamicBackground` 在配置未确定时保持背景层但不挂载默认图片；`hydrateBackend` 或明确失败后才选择自定义或默认背景。自定义背景站点不再先请求并闪现默认图；失败仍能回退默认图。没有新增 `/api` 请求。
