...
9. 详情负载图的「实时」档位逐条消费 `batchUpdate.samples[]`：样本时间兼容秒/毫秒，指标继续按 partial update 合并，每条真实样本都进入最近 10 分钟缓冲。前端不对 Angel 配置的 `wss_report_interval` 做 10 秒降采样；不同密度只按时间轴排列，超过 1 分钟没有真实样本才画断点。
