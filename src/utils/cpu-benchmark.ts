/*
 * CPU 近似分级与 PassMark 查询链接。
 *
 * 逐字移植自 Komari `src/utils/cpuBenchmark.ts`，用于详情页硬件卡的分级条与
 * 「CPU Mark 排行」外链。它是一个只读取 CPU 型号字符串的纯函数模块：
 * 不请求任何接口，也不产生 CFSM 没有的数据——分级只是按公开天梯与型号代际
 * 做的本地近似归类，上游自己的提示语同样这么说明。
 */
